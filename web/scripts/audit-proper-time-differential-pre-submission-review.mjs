...
const auditArtifacts = [
  {
    ...
    file: "web/src/data/proper-time-differential-waiting-hypothesis/REFERENCE_VERIFICATION.json",
    expectedVerdicts: ["PASS"],
  },
  {
    label: "Citation Context Review Packet",
    file: "web/src/data/proper-time-differential-waiting-hypothesis/CITATION_CONTEXT_REVIEW_PACKET.json",
    expectedVerdicts: ["READY_FOR_EXTERNAL_REVIEW"],
  },
  {
    label: "Citation Context Local Review",
    file: "web/src/data/proper-time-differential-waiting-hypothesis/CITATION_CONTEXT_LOCAL_REVIEW.json",
    expectedVerdicts: ["LOCAL_REVIEW_COMPLETE_REQUIRES_FRESH_REVIEW"],
  },
];

const requiredBoundaryStrings = [
  "不声称黑洞等待室、人工黑洞或类黑洞工程已经可行",
  "不得宣称人工黑洞可制造",
  "不输出个体永生方案",
  "不是预测器",
  "不是外部审稿结论",
  "不适合作为正式物理论文、工程可行性论文或医学/个体决策材料提交",
];
...
function renderMarkdown(audit) {
  ...
  return `# Proper-Time Differential Pre-Submission Review
...
const auditedInputHashes = {
  "web/src/data/proper-time-differential-waiting-hypothesis.json": sha256(dataContent),
  "web/src/pages/papers/proper-time-differential-waiting-hypothesis.astro": sha256(pageContent),
};
...
for (const artifact of auditArtifacts) {
  ...
  if (!fs.existsSync(artifactPath)) {
    ...
    addFinding(
      ...
      1,
      artifact.label,
      "Run npm run audit:proper-time-differential to regenerate required audit artifacts.",
    );
    continue;
  }
...
scanNeedles(`${dataContent}\n${pageContent}`, "proper-time paper source", requiredBoundaryStrings, findings);
scanBannedTerms(`${dataContent}\n${pageContent}`, "proper-time paper source", findings);
scanEmDash(pageContent, "web/src/pages/papers/proper-time-differential-waiting-hypothesis.astro", findings);
...
if (preSubmissionRows.length < 5) {
  addFinding(
    ...
    "web/src/data/proper-time-differential-waiting-hypothesis.json",
    1,
...
if (!preSubmissionRows.some((row) => row[2] === "NOT_READY_FOR_FORMAL_SUBMISSION")) {
  addFinding(
    ...
    "Keep the formal-submission boundary until external reviewer results and stronger model evidence exist.",
  );
}
...
const audit = {
  audit: "proper-time-differential-pre-submission-review",
  verdict,
  ...
  submissionRecommendation:
    "Use as a Human Infra arXiv-style working paper. Do not submit as a formal physics, engineering-feasibility, or medical decision paper until external reviewer results, stronger formal derivation, and figure/model evidence are added.",
};
...
console.log(`proper-time differential pre-submission review verdict ${verdict}`);
