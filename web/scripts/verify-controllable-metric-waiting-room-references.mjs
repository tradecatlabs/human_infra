import { createHash } from "node:crypto";
import { execFile } from "node:child_process";
import fs from "node:fs";
import path from "node:path";
import { promisify } from "node:util";
import { fileURLToPath } from "node:url";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const webRoot = path.resolve(__dirname, "..");
const execFileAsync = promisify(execFile);

const dataPath = path.join(webRoot, "src/data/controllable-metric-waiting-room-hypothesis.json");
const outputDir = path.join(webRoot, "src/data/controllable-metric-waiting-room-hypothesis");
const outputJsonPath = path.join(outputDir, "REFERENCE_VERIFICATION.json");
const outputMdPath = path.join(outputDir, "REFERENCE_VERIFICATION.md");

function sha256(content) {
  return `sha256:${createHash("sha256").update(content).digest("hex")}`;
}

function readJson(filePath) {
  const content = fs.readFileSync(filePath, "utf8");
  return { content, json: JSON.parse(content) };
}

function routeToPagePath(route) {
  const trimmed = route.replace(/^\/+|\/+$/g, "");
  if (!trimmed) {
    return path.join(webRoot, "src/pages/index.astro");
  }
  return path.join(webRoot, "src/pages", `${trimmed}.astro`);
}

function parseCurlStatus(stdout) {
  const statuses = [...stdout.matchAll(/^HTTP\/\S+\s+(\d+)/gmu)]
    .map((match) => Number.parseInt(match[1], 10))
    .filter((status) => status !== 200 || !stdout.includes("200 Connection established"));
  if (statuses.length > 0) {
    return statuses.at(-1);
  }
  const fallbackStatuses = [...stdout.matchAll(/^HTTP\/\S+\s+(\d+)/gmu)]
    .map((match) => Number.parseInt(match[1], 10));
  return fallbackStatuses.length > 0 ? fallbackStatuses.at(-1) : null;
}

async function curlHead(url) {
  try {
    const { stdout, stderr } = await execFileAsync("curl", [
      "-sS",
      "-I",
      "-L",
      "--max-time",
      "12",
      "--connect-timeout",
      "5",
      "-A",
      "human-infra-reference-verifier/0.1",
      url,
    ], { maxBuffer: 1024 * 1024 });
    return { ok: true, status: parseCurlStatus(stdout), stderr, method: "CURL_HEAD" };
  } catch (error) {
    return {
      ok: false,
      status: parseCurlStatus(error.stdout || ""),
      stderr: error.stderr || error.message,
      method: "CURL_HEAD",
    };
  }
}

async function curlGetStatus(url) {
  try {
    const { stdout, stderr } = await execFileAsync("curl", [
      "-sS",
      "-L",
      "-o",
      "/dev/null",
      "-w",
      "%{http_code}",
      "--max-time",
      "15",
      "--connect-timeout",
      "5",
      "-A",
      "human-infra-reference-verifier/0.1",
      url,
    ], { maxBuffer: 1024 * 1024 });
    return { ok: true, status: Number.parseInt(stdout.trim(), 10), stderr, method: "CURL_GET" };
  } catch (error) {
    return {
      ok: false,
      status: Number.parseInt((error.stdout || "").trim(), 10) || null,
      stderr: error.stderr || error.message,
      method: "CURL_GET",
    };
  }
}

async function verifyExternalUrl(reference) {
  const head = await curlHead(reference.url);
  const candidate = head.status && head.status >= 200 && head.status < 400
    ? head
    : await curlGetStatus(reference.url);

  if (candidate.status && candidate.status >= 200 && candidate.status < 400) {
    return {
      key: reference.key,
      title: reference.title,
      url: reference.url,
      method: candidate.method,
      liveStatus: "PASS",
      statusCode: candidate.status,
      note: "Reference URL responded successfully.",
    };
  }

  const warningStatuses = new Set([401, 403, 405, 406, 408, 429, 500, 502, 503, 504]);
  return {
    key: reference.key,
    title: reference.title,
    url: reference.url,
    method: candidate.method,
    liveStatus: candidate.status && !warningStatuses.has(candidate.status) && candidate.status >= 400 ? "FAIL" : "WARN",
    statusCode: candidate.status,
    note: candidate.status
      ? `${candidate.method} returned ${candidate.status}; human review may be needed for access-controlled or transient responses.`
      : `${candidate.method} did not return a target HTTP status: ${candidate.stderr}`,
  };
}

function verifyLocalRoute(reference) {
  const pagePath = routeToPagePath(reference.url);
  const exists = fs.existsSync(pagePath);
  return {
    key: reference.key,
    title: reference.title,
    url: reference.url,
    method: "LOCAL_ROUTE",
    liveStatus: exists ? "PASS" : "FAIL",
    statusCode: exists ? 200 : 404,
    note: exists
      ? `Local route source exists at ${path.relative(webRoot, pagePath)}.`
      : `Local route source is missing at ${path.relative(webRoot, pagePath)}.`,
  };
}

function renderMarkdown(audit) {
  const rows = audit.details.perEntry
    .map((row) => `| \`${row.key}\` | ${row.liveStatus} | ${row.method} | ${row.statusCode ?? "n/a"} | ${row.url} | ${row.note.replace(/\|/g, "\\|")} |`)
    .join("\n");

  return `# Controllable Metric Waiting Room Reference Verification

**Generated**: ${audit.generatedAt}
**Verdict**: ${audit.verdict}

${audit.summary}

This artifact checks source-entry reachability. It does not prove that a source supports a manuscript claim.

## Summary

| Status | Count |
| --- | ---: |
| PASS | ${audit.details.counts.PASS || 0} |
| WARN | ${audit.details.counts.WARN || 0} |
| FAIL | ${audit.details.counts.FAIL || 0} |

## Entries

| Reference | Status | Method | HTTP | URL | Note |
| --- | --- | --- | ---: | --- | --- |
${rows}
`;
}

const data = readJson(dataPath);
const references = (data.json.sourceRows || []).map((row) => ({
  key: row[0],
  author: row[1],
  title: row[2],
  url: row[3],
  role: row[4],
}));

const perEntry = await Promise.all(references.map(async (reference) => {
  if (!reference.url) {
    return {
      key: reference.key,
      title: reference.title,
      url: "",
      method: "NONE",
      liveStatus: "FAIL",
      statusCode: null,
      note: "Source row has no URL.",
    };
  }
  if (reference.url.startsWith("/")) {
    return verifyLocalRoute(reference);
  }
  return verifyExternalUrl(reference);
}));

const counts = perEntry.reduce((acc, entry) => {
  acc[entry.liveStatus] = (acc[entry.liveStatus] || 0) + 1;
  return acc;
}, {});
const verdict = counts.FAIL ? "FAIL" : counts.WARN ? "WARN" : "PASS";
const audit = {
  audit: "controllable-metric-waiting-room-reference-verification",
  verdict,
  reasonCode: counts.FAIL ? "reference_url_failures" : counts.WARN ? "reference_url_warnings" : "all_reference_urls_accessible",
  summary:
    counts.FAIL
      ? "At least one controllable metric waiting room reference URL failed local reachability verification."
      : counts.WARN
        ? "Controllable metric waiting room reference URLs were checked; at least one reference returned a warning that requires human review."
        : "All controllable metric waiting room reference URLs were accessible or locally present at verification time.",
  generatedAt: new Date().toISOString(),
  auditedInputHashes: {
    "src/data/controllable-metric-waiting-room-hypothesis.json": sha256(data.content),
  },
  details: {
    referenceCount: references.length,
    counts,
    perEntry,
  },
};

fs.mkdirSync(outputDir, { recursive: true });
fs.writeFileSync(outputJsonPath, `${JSON.stringify(audit, null, 2)}\n`, "utf8");
fs.writeFileSync(outputMdPath, renderMarkdown(audit), "utf8");

console.log(`controllable metric waiting room reference verification verdict ${verdict}`);
console.log(`references ${references.length}`);
console.log(`pass ${counts.PASS || 0}`);
console.log(`warn ${counts.WARN || 0}`);
console.log(`fail ${counts.FAIL || 0}`);
console.log(`wrote ${path.relative(webRoot, outputJsonPath)}`);
console.log(`wrote ${path.relative(webRoot, outputMdPath)}`);

if (verdict === "FAIL") {
  process.exitCode = 1;
}
