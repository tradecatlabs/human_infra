...
function renderMarkdown(audit) {
  ...
  return `# Proper-Time Differential Reference Verification
...
const references = (data.json.referenceRows || []).map((row) => ({
  key: row[0],
  title: row[1],
  url: row[2],
  role: row[3],
...
const audit = {
  audit: "proper-time-differential-reference-verification",
  verdict,
  reasonCode: counts.FAIL ? "reference_url_failures" : counts.WARN ? "reference_url_warnings" : "all_reference_urls_accessible",
  summary:
    counts.FAIL
      ? "At least one proper-time differential reference URL failed local reachability verification."
      : counts.WARN
        ? "Proper-time differential reference URLs were checked; at least one reference returned a warning that requires human review."
...
console.log(`proper-time differential reference verification verdict ${verdict}`);
