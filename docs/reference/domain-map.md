...
- `mental-health-affective-stability`、`sleep-circadian-recovery` 和 `pain-suffering-control` 关注存在时间是否能转化为可承受、可恢复、可行动的有效时间。
- `biostasis-cryopreservation` 关注暂停损耗并等待未来修复，与 `future-waiting` 的时间差分路径互补。
- `risk-engineering`、`planetary-health-environment`、`resource-social-infra` 和 `governance-rights` 关注主体不被外部风险、资源断裂、环境退化或制度控制截断。
