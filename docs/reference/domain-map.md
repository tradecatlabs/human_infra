...
- `risk-engineering`、`planetary-health-environment`、`resource-social-infra`、`social-determinants-community-vulnerability`、`public-service-design-accessibility`、`emergency-alerts-communications`、`disaster-recovery-relief-continuity`、`financial-inclusion-payment-systems`、`workforce-employment-services`、`social-connection-relational-infra`、`health-literacy-navigation` 和 `governance-rights` 关注主体不被外部风险、资源断裂、结构性脆弱性、服务不可达、危机通信失败、灾后恢复失败、支付入口失败、就业入口失败、环境退化、关系孤立、服务不可理解或制度控制截断。
